    ...
    <v-content>
      ...
      <div class="text-center">
        <v-dialog v-model="dialog" width="500">
          <v-card>
            <v-card-title class="headline blue" style="color:white">
              <v-icon class="mr-1" color="white">mdi-party-popper</v-icon>Bingo!
            </v-card-title>

            <v-card-text>
              <br />
              <h1 class="text-center">Alguien ganó :v</h1>
            </v-card-text>
...
export default {
  ...
  data() {
    return {
      ...
      isMyTurn: false,
      canReset: false,
      usedNumbers: new Array(75),
      usedNumbers1: new Array(75),
      calledNumbers: new Array(),
      calledNumbersLetters: new Array(),
      dialog: false
    };
  },
  watch: {
    ...
    isMyTurn: function() {
      if (this.isMyTurn && this.player.settings.auto) this.sacarNum();
    }
  },
  sockets: {
    youTurn() {
  ...
  methods: {
    ...
    colorSquare(squareNum, carton) {
      let card = carton - 1;
      if (squareNum == 0) {
        let searchStr = "N" + (carton + 1) + "3";
        console.warn(searchStr);
        let currentSquare = document.getElementById(searchStr);
    ...
    checks(carton) {
      console.error("Verificando" + " " + carton);
      if (this.player.settings.mode == "linea") {
        if (
          ...
          console.error("GANE AQUI " + carton);
          this.win = true;
        }
      }
    ...
    checkN(carton) {
      return (
        this.calledNumbers.includes(this.card[carton].N[0]) &&
        this.calledNumbers.includes(this.card[carton].N[0]) &&
        this.calledNumbers.includes(this.card[carton].N[1]) &&
        this.calledNumbers.includes(this.card[carton].N[3]) &&
        this.calledNumbers.includes(this.card[carton].N[4])
      );
    },
    checkG(carton) {
      return (
    ...
    resetThis() {
      this.usedNumbers = new Array(76);
      this.usedNumbers1 = new Array(76);
      this.calledNumbers = new Array();
      this.calledNumbersLetters = new Array();
      this.canReset = false;
      this.win = false;
      for (var k = 0; k < parseInt(this.player.settings.cards); k++)
        this.generateAnotherCard(k);

      this.$emit("updateView", "Mode");
    }
  },
  created() {
    for (var k = 0; k < parseInt(this.player.settings.cards); k++)
